...
impl<D: RamenDriver> TouchSession<D> {
    ...
    pub fn submit(&mut self, index: usize) -> Result<(), SubmitError> {
        ...
        self.state = match self.driver.submit(index)? { PendingDecisionOrFinished::Decision(d) => MobileState::Decision(d), PendingDecisionOrFinished::Finished(s) => MobileState::Finished(s) };
        Ok(())
    }
    fn advance(&mut self) -> Result<(), String> {
        self.state = match self.driver.advance_until_decision()? { PendingDecisionOrFinished::Decision(d) => MobileState::Decision(d), PendingDecisionOrFinished::Finished(s) => MobileState::Finished(s) };
        Ok(())
    }
}
...
mod tests {
    use super::*;
    #[test] fn ui_state_exposes_pending_decision() {
        let d = PendingDecision { turn: 2, kind: DecisionKind::Ramen, title: "选择拉面".into(), options: vec![DecisionOption { index: 0, title: "不吃面".into(), detail: String::new() }] };
        let state = MobileState::Decision(d);
        assert_eq!(UiState::from(&state).status, UiStatus::Decision);
    }
}
