...
impl RamenGamePort for RamenGameAdapter {
    ...
    fn choose(&mut self, index: usize) -> Result<PortResult, String> {
        ...
        match captured {
            Capture::Action { actions, .. } => { if index >= actions.len() { return Err(format!("动作索引越界: {} / {}", index, actions.len())); } let action = actions[index]; let mut game = self.game.take().ok_or_else(|| "游戏尚未初始化".to_string())?; action.apply(&mut game, &mut self.rng).map_err(|e| e.to_string())?; self.game = Some(game); self.game_mut()?.next(); }
            Capture::Event { event, choices } => { if index >= choices.len() { return Err(format!("事件选项索引越界: {} / {}", index, choices.len())); } let game = self.game_mut()?; game.apply_event(&event, index, &mut self.rng).map_err(|e| e.to_string())?; }
        }
        self.pump_inner()
    }
}
