...
//!
//! 关键点：`RamenGame::run_stage` 原本会通过 `Trainer` 阻塞等待输入；这里的
//! `PauseTrainer` 在遇到玩家决策时捕获候选并返回一个内部暂停错误。适配器保留
//! `RamenGame` 原状态，收到手机索引后再应用原始动作/事件选择。
...
use umasim::game::ramen::{Operation, RamenAction, RamenGame, RamenStage};
use umasim::game::{ActionEnum, EventChoice, EventData, Game, Trainer};
...
enum Capture {
    ...
    Event { event: EventData, choices: Vec<Vec<EventChoice>> },
}
...
impl RamenGameAdapter {
    ...
    fn decision(capture: &Capture, turn: i32) -> PendingDecision {
        match capture {
            Capture::Action { kind, actions } => PendingDecision {
                turn: turn as u32,
                kind: kind.clone(),
                title: match kind { DecisionKind::Ramen => "选择拉面", DecisionKind::SpecialFeeling => "选择隐藏风味用法", DecisionKind::Training => "选择行动", DecisionKind::Region => "选择地区", DecisionKind::SuperRamen => "选择超级拉面", DecisionKind::Event => "选择事件" }.into(),
                options: action_options(actions),
            },
            Capture::Event { event, choices } => PendingDecision {
                turn: turn as u32,
                kind: DecisionKind::Event,
                title: if event.name.is_empty() { "选择事件选项".into() } else { event.name.clone() },
                options: choices.iter().enumerate().map(|(index, group)| PendingDecisionOption::from_event(index, group)).collect(),
            },
        }
    }
    ...
    fn pump_inner(&mut self) -> Result<PortResult, String> {
        loop {
            ...
            let trainer = PauseTrainer { capture };
            let result = {
                let game = self.game_mut()?;
                game.run_stage(&trainer, &mut self.rng)
            };
            if let Some(captured) = self.capture.borrow_mut().take() {
                let turn = self.game()?.turn();
                return Ok(PortResult::Decision(Self::decision(&captured, turn)));
            }
            if let Err(error) = result { return Err(error.to_string()); }
            let more = self.game_mut()?.next();
            if !more { return Ok(PortResult::Finished(self.summary())); }
        }
    }

    fn summary(&self) -> GameSummary {
        let game = self.game.as_ref().expect("summary requires game");
        GameSummary { final_turn: game.turn() as u32, score: Some(game.uma.calc_score()), pt: Some(game.ramen.scenario_pt) }
    }
}

struct PendingDecisionOption;
impl PendingDecisionOption {
    fn from_event(index: usize, group: &[EventChoice]) -> crate::DecisionOption {
        crate::DecisionOption { index, title: format!("选项 {}", index + 1), detail: format!("{:?}", group) }
    }
}
