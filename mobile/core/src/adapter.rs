...
impl RamenGameAdapter {
    pub fn new(config: RamenMobileConfig) -> Self { Self { game: None, rng: StdRng::seed_from_u64(config.seed), capture: Arc::new(Mutex::new(None)), config } }
    ...
    fn pump_inner(&mut self) -> Result<PortResult, String> { if self.game.is_none() { return Err("游戏尚未初始化".into()); } loop { let trainer = PauseTrainer { capture: self.capture.clone() }; let result = { let game = self.game.as_mut().ok_or_else(|| "游戏尚未初始化".to_string())?; let rng = &mut self.rng; game.run_stage(&trainer, rng) }; let captured = self.capture.lock().expect("capture mutex").clone(); if let Some(captured) = captured { let turn = self.game()?.turn(); return Ok(PortResult::Decision(Self::decision(&captured, turn))); } if let Err(error) = result { return Err(error.to_string()); } if !self.game.as_mut().ok_or_else(|| "游戏尚未初始化".to_string())?.next() { return Ok(PortResult::Finished(self.summary())); } } }
...
impl RamenGamePort for RamenGameAdapter {
    fn initialize(&mut self) -> Result<(), String> { let (game, rng) = create_ramen_game(self.config.clone()).map_err(|e| e.to_string())?; self.game = Some(game); self.rng = rng; self.capture.lock().expect("capture mutex").take(); Ok(()) }
    fn pump(&mut self) -> Result<PortResult, String> { self.pump_inner() }
    fn choose(&mut self, index: usize) -> Result<PortResult, String> { let captured = self.capture.lock().expect("capture mutex").take().ok_or_else(|| "当前没有待决策".to_string())?; match captured { Capture::Action { actions, .. } => { if index >= actions.len() { return Err(format!("动作索引越界: {} / {}", index, actions.len())); } let action = actions[index]; let mut game = self.game.take().ok_or_else(|| "游戏尚未初始化".to_string())?; action.apply(&mut game, &mut self.rng).map_err(|e| e.to_string())?; self.game = Some(game); if !self.game.as_mut().ok_or_else(|| "游戏尚未初始化".to_string())?.next() { return Ok(PortResult::Finished(self.summary())); } }, Capture::Event { event, choices } => { if index >= choices.len() { return Err(format!("事件选项索引越界: {} / {}", index, choices.len())); } let game = self.game.as_mut().ok_or_else(|| "游戏尚未初始化".to_string())?; let rng = &mut self.rng; game.apply_event(&event, index, rng).map_err(|e| e.to_string())?; // run_stage 已因暂停退出；推进到下一阶段，避免从 Begin/AfterTrain 重新执行整个阶段。
            if !game.next() { return Ok(PortResult::Finished(self.summary())); }
        } } self.pump_inner() }
}
